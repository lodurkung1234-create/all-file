...
local UI_ListLabel = ListGroup:AddLabel("ยังไม่มีรายการ")
...
local function updateUI()
    local str = ""
    if #getgenv().BuyList == 0 then
        str = "ยังไม่มีรายการ"
    else
        for i, v in ipairs(getgenv().BuyList) do
            str = str .. string.format("%d. %s | %s | %s\n", i, tostring(v.Name), tostring(v.Rarity), tostring(v.Mutation))
        end
    end
    UI_ListLabel:SetText(str)
